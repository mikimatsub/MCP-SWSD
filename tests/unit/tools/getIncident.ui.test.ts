...
describe('swsd_get_incident UI registration', () => {
  it("attaches a ui:// resourceUri to the registered tool's _meta and registers the resource", async () => {
    ...
    // implementation registers. A typo in 'incident-detail' would no longer
    // slip through.
    const modernUri = incident._meta?.ui?.resourceUri;
    expect(modernUri).toBe('ui://swsd/incident-detail.html');
    ...
    // Exercise the read callback so the loadUiResource('incident-detail')
    ...
    expect(result.contents[0]?.text).toContain('SWSD Incident'); // sentinel from the bundled HTML <title>
