...
The middle box. Single Node.js process, TypeScript, six direct production dependencies (`@modelcontextprotocol/sdk`, `@modelcontextprotocol/ext-apps`, `express`, `express-rate-limit`, `zod`, `dompurify`). Exposes [35 tools](/tools/) organized into [profiles](/configuration/#profiles).
